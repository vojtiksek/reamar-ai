 ...
 export default function UnitDetailPage() {
   ...
   const editableColumns = useMemo(() => {
     if (!columnsState.data) return [] as UnitColumn[];
     const cols = columnsState.data.filter(
       (c) =>
         c.editable &&
         (c.entity === "unit" || !c.entity) &&
         OVERRIDEABLE_UNIT_FIELDS.has(c.key)
     );
   ...
   const handleSave = async () => {
     ...
     try {
       const changedColumns = editableColumns.filter((col) => {
         ...
         const prevVal = originalUnit[key];
       ...
       for (const col of changedColumns) {
         ...
         const res = await fetch(
           `${API}/units/${encodeURIComponent(id)}/overrides/${encodeURIComponent(key)}`,
           {
             method: "PUT",
         ...
         if (!res.ok) {
           ...
           throw new Error(text || `Failed to save unit override for ${key}`);
         }
