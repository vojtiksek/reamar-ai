...
const DEFAULT_HIDDEN_COLUMN_KEYS = new Set<string>([
  ...
  "total_units",
  "available_units",
  // Ceny stání / garáže – nyní trvale skryté v HIDDEN_TABLE_COLUMN_KEYS
  // Změna ceny
  "price_change",
  // Podíl dostupných
  "availability_ratio",
  ...
  const handleRowClick = useCallback(
    (e: React.MouseEvent<Element>, u: Unit) => {
      ...
      if (e.metaKey || e.ctrlKey || e.button === 1) {
        window.open(`/explorer/units/${encodeURIComponent(externalId)}`, "_blank", "noopener,noreferrer");
        return;
      }
      ...
      if (e.detail > 1) {
        ...
        router.push(`/explorer/units/${encodeURIComponent(externalId)}`);
        return;
      }
                          ...
                          <td colSpan={visibleColumns.length + (activeClient ? 1 : 0)} className="p-0">
                            <div className="sticky left-0 w-[calc(100vw-2rem)] max-w-[calc(100vw-2rem)] overflow-hidden border-b border-slate-200 bg-slate-50/90 px-5 py-4">
                              ...
                              <div className="mt-3 flex gap-2">
                                ...
                                {u.project && Boolean((u.project as Record<string, unknown>).id) && (
                                  <a
                                    href={`/projects/${String((u.project as Record<string, unknown>).id)}`}
