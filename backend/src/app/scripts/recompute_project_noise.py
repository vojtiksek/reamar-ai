...
"""
Batch recompute of project noise (denní / noční hluk) from Prague noise map polygons.

Usage (from backend/ with venv active):

    python -m app.scripts.recompute_project_noise
"""
...
from app.noise import compute_project_noise


def main(batch_size: int = 200) -> None:
    start = time.perf_counter()
    print(f"Recomputing project noise (batch_size={batch_size})")
    ...
    print(f"Done. Recomputed noise for {processed} projects in {total_elapsed:.2f}s.")
